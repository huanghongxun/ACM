...
#include <algorithm>
#define N 200005
int sz[N], l[N], r[N], fa[N], sum[N], val[N], max[N], rev[N], sk[N], min[N];

void update(int x) {
	max[x] = std::max(std::max(max[l[x]], max[r[x]]), val[x]);
	min[x] = std::min(std::min(min[l[x]], min[r[x]]), val[x]);
	sum[x] = sum[l[x]] + sum[r[x]] + val[x];
}

void reverse(int x, int y) {
	makeroot(x); access(y); splay(y);
	sum[y] = -sum[y]; swap(max[y], min[y]);
...
int h[N], p[N], to[N], cnt = 0, vis[N];
void add(int a, int b) {
	p[cnt] = h[a],  to[cnt] = b, h[a] = cnt++;
}
